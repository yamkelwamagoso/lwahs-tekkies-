...
.price{color:#ff1493;font-weight:bold;font-size:16px}
.btn-add{background:#ff1493;color:white;padding:9px;border:none;border-radius:20px;width:100%;margin-top:6px;font-weight:bold}
.btn-wa{background:#25D366;color:white;padding:9px;border:none;border-radius:20px;width:100%;margin-top:6px;font-weight:bold}
...
.modal img{max-width:92%;max-height:70%;border-radius:15px;border:3px solid white}
...
.view-hint{font-size:11px;color:#ff69b4;margin-top:3px}
</style></head><body>
<div class="header"><div class="logo">💖 Lwah's Tekkies</div><div class="nav"><a href="/">SHOP</a> <a href="#" onclick="checkoutWA()">🛒 Cart (<span id="cartCount">0</span>)</a></div></div>
<div class="banner"><h2 style="margin:0">Step Out. Stand Out. 👑</h2><p style="margin:5px 0;font-size:13px">Tap picture to view • Add to cart • WhatsApp to order</p></div>
<div class="products">
{% for p in products %}
<div class="card">
<img src="{{p.image}}" onclick="openModal('{{p.image}}','{{p.name}} - R{{p.price}}')">
<div class="view-hint">👁️ Tap to view big</div>
<h4 style="margin:6px 0;font-size:14px">{{p.name}}</h4>
<div class="price">R{{p.price}}</div>
<p style="font-size:11px;color:#666;margin:3px">{{p.desc}}</p>
...
<div id="imgModal" class="modal" onclick="closeModal()"><span class="close">&times;</span><img id="modalImg"><h3 id="modalTitle"></h3><p style="color:#ffb6d9">Tap anywhere to close</p></div>
<div class="footer">© Lwah's Tekkies | 081 566 6133 | Durban 💖<br>Click picture to view big!</div>
<script>
...
function checkoutWA(){if(cart.length==0){alert('Cart empty queen! Add tekkies first 👑');return;}let msg='Hi Lwah! 👑 I want to order:%0A';let total=0;cart.forEach((it,i)=>{msg+=`${i+1}. ${it.name} - R${it.price}%0A`;total+=parseInt(it.price)||0;});msg+=`%0ATotal: R${total}%0AFrom: Lwahs Tekkies Website`;window.open('https://wa.me/27815666133?text='+msg,'_blank');}
</script></body></html>
"""
...
</head><body><h2 style="color:#ff1493">🔐 Lwah Secret Admin</h2><p>Only you know this link!</p>
<form method="post" enctype="multipart/form-data">
<input name="name" placeholder="Product Name" required><input name="price" placeholder="Price e.g 1200" required>
<textarea name="desc" placeholder="Size / Info"></textarea><input type="file" name="image" accept="image/*" required>
<button class="btn" type="submit">Upload Product 👑</button></form><br><a href="/">← Back to Shop</a></body></html>"""
